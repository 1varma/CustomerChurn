...
**Project Overview:** This project analyzes customer data to predict **churn** – the event of a customer leaving a service.  In a telecom context, predicting churn lets a company identify at‑risk customers so they can be retained (which is far more cost‑effective than acquiring new ones). By building a machine learning model on historical data, we can flag customers likely to cancel service and take proactive retention actions (e.g. targeted offers). Reducing churn has a direct impact on revenue and customer satisfaction.
